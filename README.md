...
> 再解釈型創造性候補とは、主体の内部ワールドモデルに生じた不整合が、より整合的な代替モデルへ修復・変形され、その代替モデルが元のモデルと異なり、明示された構造的 proxy によって評価される場合に成立しうる候補である。
...
3. 再解釈後モデルは元のモデルと異なる。
4. 再解釈後モデルは、明示された構造的 proxy によって評価される。
...
- utility_proxy が人間の価値判断を捉える。
- 対話・共通理解・行動リスク低減の一般理論を作る。

対話や共通理解は背景概念として重要ですが、このリポジトリの主対象は **限定された再解釈モデル** です。

## 最小モデル

初期ワールドモデルを `W`、再解釈後モデルを `W'` とします。

再解釈型創造性候補 `RIC_candidate(W, W')` を、次のように定義します。
...
RIC_candidate(W, W') :=
  ...
  and Eval_proxy(W')  // W' は明示された構造的 proxy で評価される
```

ここで重要なのは、**誤りそのものを創造性とは呼ばない**ことです。

誤り・不整合・曖昧さは、再解釈を発生させるきっかけになりえます。しかし、それが創造性候補になるには、少なくとも整合性・差分・明示された proxy 上の評価が必要です。
...
- 整合性、新規性、保存度、utility_proxy による評価
...
この結果は、創造性全体を証明するものではありません。局所修復探索はノード数4〜5で再解釈探索より高い proxy score を出していますが、これは現行スコア関数と候補生成条件に依存します。
...
代表ケースは、可能な限り同じ `node_count / seed` を重複利用しないように選ばれます。候補が足りない場合だけ重複を許容し、その場合は `duplicate_allowed` に記録します。

この抽出は説明補助であり、創造性の証明ではありません。

## 主要文書

- [`docs/model_core.md`](docs/model_core.md): 現行モデルの中核
- [`docs/formal_assumptions.md`](docs/formal_assumptions.md): 形式的前提
- [`docs/closure_criteria.md`](docs/closure_criteria.md): current-scope closure 条件
...
目的は、再解釈型創造性の限定モデルを、過剰主張を避けながら、形式化・シミュレーション・検証可能な仮説として整理することです。
